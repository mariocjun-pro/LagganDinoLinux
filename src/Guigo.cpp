...
void Guigo::executar() {
    if (!ataquePronto) { totalT += pGG->getDt(); }
    ...
    if (Keyboard::isKeyPressed(tecla) && ataquePronto) {
        ...
        if (abs(movimento.x) > 0) { vJog = velocidade; }
        else { vJog = 0; }
    }
    ...
    hitbox->getCorpoGraf()->mover((298.0f + vJog) * pGG->getDt() * static_cast<float>(ladoAtaque), 0.0f);
    if (atacando) {
        hitbox->getCorpoGraf()->getAnimadora()->atualizarLinhasSequencial(pGG->getDt(), aDireita,
                                                                          Vector2u(8, 8), 0.1f);
    }
    mover();
}
...
void Guigo::mover() {
    ...
    if (Keyboard::isKeyPressed(Keyboard::Key::Space) && noChao) {
        noChao = false;
        movimento.y = -sqrtf(static_cast<float>(2.0 * 981.0 * pulo));
    }

    movimento.y += static_cast<float>(981.0 * dT * 2.5f);
