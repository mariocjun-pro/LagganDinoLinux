...
void ListaEntidade::colidir(Jogador *j) {
    ...
    while (itr) {
        ...
        if (GerenciadoresFases::Colisora::atacando(itr->getAtual(), jog, direcao)) {
            if (itr->getAtual()->getPlataforma()) {
                GerenciadoresFases::Colisora::colidir(reinterpret_cast<Entidade *>(itr->getAtual()->getPlataforma()),
                                                      itr->getAtual(),
                                                      direcao);
            }
            GerenciadoresFases::Colisora::colidir(itr->getAtual(), jog, direcao);
            if (j->getAtacando())
                if (GerenciadoresFases::Colisora::ataque(itr->getAtual(), static_cast<Entidade *>(j->getProjetil()),
                                                         direcao)) {
...
void ListaEntidade::colidir(Jogador *j, Jogador *j2) {
    ...
    while (itr) {
        ...
        if (GerenciadoresFases::Colisora::atacando(itr->getAtual(), jog2, direcao)) {
            if (itr->getAtual()->getPlataforma())
                GerenciadoresFases::Colisora::colidir(reinterpret_cast<Entidade *>(itr->getAtual()->getPlataforma()),
                                                      itr->getAtual(),
                                                      direcao);

            GerenciadoresFases::Colisora::colidir(itr->getAtual(), jog2, direcao);
            if (j2->getAtacando())
                if (GerenciadoresFases::Colisora::ataque(itr->getAtual(), static_cast<Entidade *>(j2->getProjetil()),
                                                         direcao)) {
                    if (itr->getAtual()->tomarDano()) {
